#!/bin/bash
reposToScan=("deprecation-test") # in case of multiple repos, separate them with whitespace, f.ex. ("repo1" "repo2" "repo3")
...
for repo in "${reposToScan[@]}"; do
    ...
    printf "\n*********** $repo ************\n"
    printf "\nYAML TEMPLATES:\n"
    detect-files -d "$repoDir" -o markdown --ignore-deprecations --ignore-removals # 2 last parameters can be removed if you want script to fail in case deprecations/removals have been detected
